...
description: >-
  ...
  fixes. Used by /architect skill during planning phase.
...
The work plan JSON will be provided in your prompt by the /architect skill.
...
### Scope Issues
- Tasks too large for one session (>5 files or >3 concerns)
...
- Tasks where the LLM will hallucinate libraries that don't exist
- Tests that pass without testing real behavior (mocking everything)
- Acceptance criteria that can be satisfied with hardcoded values
